<template>
  ...
  >
    ...
    <div
      class="md-image-review_controller-fix left"
      @click.stop="checkPic(-1)"
    >
      <i class="icon iconfont icon-xiangzuojiantou" />
    </div>
    <div
      class="md-image-review_controller-fix right"
      @click.stop="checkPic(1)"
    >
      <i class="icon iconfont icon-xiangyoujiantou" />
    </div>
    <div class="md-image-review_controller-fix close">
      <i class="icon iconfont icon-guanbi" />
    </div>
    <div
      class="md-image-review_controller"
      @click.stop
    >
      <div
        class="md-image-review_controller-item"
        @click="scaleHandler(-1)"
      >
        <i class="icon iconfont icon-suoxiao1" />
      </div>
      <div
        class="md-image-review_controller-item"
        @click="scaleHandler(1)"
      >
        <i class="icon iconfont icon-fangda1" />
      </div>
      <div
        class="md-image-review_controller-item"
        @click="rotateHandler(-1)"
      >
        <i class="icon iconfont icon-nishizhenxuanzhuan" />
      </div>
      <div
        class="md-image-review_controller-item"
        @click="rotateHandler(1)"
      >
        <i class="icon iconfont icon-shunshizhenxuanzhuan" />
      </div>
      <div
        class="md-image-review_controller-item"
        @click="reflushHandler"
      >
        <i class="icon iconfont icon-zhongzhi" />
      </div>
    </div>
    <div
      v-show="loading"
      class="md-image-review_loading"
    >
      <svg
        id="L9"
        version="1.1"
...
export default defineComponent({
  ...
  setup() {
    ...
    const computedSize = () => {
      ...
      res.left -= scaleParams.type * offset.left;
      res.top -= scaleParams.type * offset.top;
      if (
